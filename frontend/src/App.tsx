...
function App() {
  ...
  return (
    <div className="mx-auto min-h-svh max-w-7xl px-4 pb-12 pt-4 sm:px-6 lg:px-8">
      <header className="rounded-[24px] border border-slate-200 bg-white/95 p-3 shadow-sm backdrop-blur sm:p-4">
        <div className="flex items-center justify-between gap-3">
          <Link to="/" className="shrink-0">
            <PikmeLogo className="h-8 w-auto sm:h-9" />
          </Link>

          <div className="mx-auto hidden rounded-full border border-slate-200 bg-slate-50 p-1 sm:flex">
            <button
        ...
        <div className="mt-3 flex rounded-full border border-slate-200 bg-slate-50 p-1 sm:hidden">
          <button
            onClick={() => setEntityFilter('SALON')}
            className={`flex-1 rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
              entityFilter !== 'MASTER' ? 'bg-brand-600 text-white' : 'text-slate-600'
            }`}
          >
            Салоны
          </button>
